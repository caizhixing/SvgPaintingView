package xyz.herelookingatyoukid.svgpainting.core.helper.map;
...
public class SVGParser {
    ...
    public static class SVGHandler extends DefaultHandler {
        ...
        public ArrayList<PathBean> pathBeans = new ArrayList<>();
        // svg的g标签对应的id名
        ...
        public Picture reDraw() {
            this.canvas = this.picture.beginRecording(1024, 1024);
            this.canvasRestoreCount = this.canvas.save();
            float f = (float) 1024;
            this.canvas.clipRect(0.0f, 0.0f, f, f);
            for (int i = 0; i < this.pathBeans.size(); i++) {
                PathBean pathItem = this.pathBeans.get(i);
                this.fillPaint.setColor(pathItem.getFillColor());
                if (pathItem.getRect() == null) {
                    this.canvas.drawPath(pathItem.getPath(), this.fillPaint);
                } else if (pathItem.getLocalName().equals("circle") || pathItem.getLocalName().equals("ellipse")) {
                    this.canvas.drawOval(pathItem.getRect(), this.fillPaint);
                } else {
                    this.canvas.drawRect(pathItem.getRect(), this.fillPaint);
                }
            }
            if (this.canvasRestoreCount != null) {
                this.canvas.restoreToCount(this.canvasRestoreCount);
            }
        ...
        public void startElement(String namespaceURI, String localName, String qName, Attributes atts) throws SAXException {
            ...
            if (localName.equals("svg")) {
                ...
                if (SVGParser.getStringAttr("id", atts) != null) {
                    String access$1002 = SVGParser.getStringAttr("id", atts);
                    if (access$1002 != null && !access$1002.equals("")) {
                        this.groupName = SVGParser.getStringAttr("id", atts);
                        this.opacity = 1.0f;
                    }
                }
                String opacityAttr = SVGParser.getStringAttr("opacity", atts);
                if (opacityAttr != null) {
                    this.opacity *= convertToFloat(opacityAttr, 1.0f);
                }
                if (hidden) {
                    hiddenLevel++;
                ...
                if (doFill(props, rect)) {
                    ...
                    int o = (int) (this.opacity * 255.0f);
                    int c = this.fillPaint.getColor();
                    this.fillPaint.setColor(Color.argb(0, (c >> 16) & 255, (c >> 8) & 255, (c) & 255));
                    saveToPathBean(this.fillPaint, rect, groupName, o, localName);
                    doLimits(rect);
                }
                if (doStroke(props)) {
                    rect.set(x, y, x + width, y + height);
                ...
                if (centerX != null && centerY != null && radiusX != null && radiusY != null) {
                    ...
                    if (doFill(props, rect)) {
                        ...
                        this.fillPaint.getColor();
                        int i4 = (int) (this.opacity * 255.0f);
                        this.canvas.drawOval(this.rect, this.fillPaint);
                        saveToPathBean(this.fillPaint, this.rect, this.groupName, i4, localName);
                    }
                    if (doStroke(props)) {
                        canvas.drawOval(rect, strokePaint);
                        doLimits(rect, strokePaint);

                        int i5 = (int) (this.opacity * 255.0f);
                        this.canvas.drawOval(this.rect, this.strokePaint);
                        saveToPathBean(this.strokePaint, this.rect, this.groupName, i5, localName);
                    }
                    popTransform();
                }
                ...
                if (numbers != null) {
                    ...
                    if (points.size() > 1) {
                        ...
                        saveToPathBean(this.fillPaint, p, this.groupName, (int) (this.opacity * 255.0f));
                        popTransform();
                    }
                }
        ...
        private LayerAttributes currentLayerAttributes() {
            return layerAttributeStack.getLast();
        }


        private void saveToPathBean(Paint paint, RectF rectF, String groupName, int i, String localName) {
            RectF rectF2;
            ...
            if (i3 == 0 && i4 == 0 && i5 == 0) {
                pathBean.setEnableFillColor(false);
                if (i2 == 255) {
                    pathBean.setStroke(true);
                } else {
                    pathBean.setStroke(false);
                }
            } else {
                pathBean.setEnableFillColor(true);
            }
            if (groupName.toLowerCase().contains("shade")) {
                pathBean.setShade(true);
                pathBean.setEnableFillColor(false);
            }
            pathBean.setGroupName(groupName);
            if (i < 255) {
                pathBean.setShade(true);
            } else {
                pathBean.setShade(false);
            }
            pathBean.setLocalName(localName);
            if (!groupName.toLowerCase().contains("bg")) {
                rectF2 = new RectF(rectF.left, rectF.top, rectF.right, rectF.bottom);
            } else {
                rectF2 = new RectF(0.0f, 0.0f, 1024.0f, 1024.0f);
            }
            pathBean.setFillColor(paint.getColor());
            pathBean.setRect(rectF2);
            this.pathBeans.add(pathBean);
        }

        private void saveToPathBean(Paint paint, Path path, String groupName, int opacity) {
            int color = paint.getColor();
            int i2 = (color >> 24) & 255;
            int i3 = (color >> 16) & 255;
            int i4 = (color >> 8) & 255;
            int i5 = (color) & 255;
            PathBean pathItem = new PathBean();
            if (i3 == 0 && i4 == 0 && i5 == 0) {
                pathItem.setEnableFillColor(false);
                if (i2 == 255) {
                    pathItem.setStroke(true);
                } else {
                    pathItem.setStroke(false);
                }
            } else {
                pathItem.setEnableFillColor(true);
            }
            if (groupName.toLowerCase().contains("shade")) {
                pathItem.setEnableFillColor(false);
            }
            pathItem.setGroupName(groupName);
            if (opacity < 255) {
                pathItem.setShade(true);
            } else {
                pathItem.setShade(false);
            }
            pathItem.setFillColor(paint.getColor());
            pathItem.setPath(path);
            this.pathBeans.add(pathItem);
        }
